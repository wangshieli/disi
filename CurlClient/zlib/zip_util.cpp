...
void change_file_date( const char *filename,uLong dosdate,tm_unz tmu_date)
{
  ...
  hFile = CreateFileW(filenamew,GENERIC_READ | GENERIC_WRITE,
...
int mymkdir(const char* dirname)
{
	...
	int res = CreateDirectoryW(dirnamew,NULL);
	free(dirnamew);
    return res;
}
	...
	int* popt_overwrite,const char* password,const char* dir)
{
	...
	strncpy_s(filename_inzip,dir1.c_str(),sizeof(filename_inzip));
	...
	else
	{
		...
		{
			_wfopen_s(&fout,write_filenamew,L"wb");
...
//    unz_file_info file_info;          // for unzGetCurrentFileInfo
	...
	CreateDirectoryW(unzip_dir.c_str(),NULL);
